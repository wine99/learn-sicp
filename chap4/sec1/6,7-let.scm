...
; '<text-of-quotation> -> (quote <text-of-quotation>)
(define (quoted? exp)
...
; the argument of true? and false? should be value, not expression
; aka, (true? (eval <exp>))
; eg. (true? (eval 1)), (true? (eval 'true)), not (true? 'true)
(define (true? x)
  (not (eq? x #f)))
(define (false? x)
  (eq? x #t))
...
(define (let*->let exp)
  (define (reduce-let* lets body)
    (if (null? (cdr lets))
        ...
        (list 'let
              (list (car lets))
              (reduce-let* (cdr lets) body))))
  (reduce-let* (cadr exp) (cddr exp)))
; -----------------------------------------------
...
(eval '(let* ((x 1)) x) the-global-environment)
(eval '(let* ((x 1) (y (cons x 2))) y) the-global-environment)
